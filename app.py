...
def validate_numeric_input(value, min_val, max_val, var_name):
    """Vérifie si la valeur est comprise entre min_val et max_val.
    Affiche une alerte si ce n'est pas le cas."""
    if not (min_val <= value <= max_val):
        st.warning(f"La valeur de '{var_name}' doit être comprise entre {min_val} et {max_val}.")
    return value

# ===============================
# Chargement et validation des données d'entraînement
# ===============================
def load_training_data(file):
    """Charge le fichier CSV et vérifie que toutes les colonnes requises sont présentes."""
    required_columns = [
...
def train_models(X, y):
    """Entraîne les modèles de classification et renvoie les modèles ainsi que leur précision."""
...
# Stockage en session state pour ne pas réentraîner inutilement
...
def predict_match(
    ...
    # Ici, vous pouvez optimiser les pondérations via une méthode d'apprentissage ou d'optimisation.
    note_offensive_A = (
        xG_A * 0.2 +
        tirs_cadrés_A * 0.15 +
...
with st.expander("📖 Comment remplir les données ?"):
    st.markdown("""
    - **Les statistiques doivent être saisies comme des moyennes par match.**
    - Par exemple, entrez le xG moyen, le nombre moyen de tirs cadrés, etc.
    - Pour les taux (conversion, possession), saisissez des nombres décimaux (ex : 30.00, 55.00).
    - Vérifiez que toutes les valeurs sont dans des plages raisonnables (ex : possession entre 0 et 100).
    """)

# Saisie des données pour l'équipe A (domicile)
col1, col2 = st.columns(2)

with col1:
    st.header("🏠 Équipe A (Domicile)")
    # Utiliser des inputs pour obtenir des float
...
if st.button("🔮 Prédire le Résultat"):
    ...
    # Entraînement / chargement des modèles de classification
    if st.session_state.get("models") is None:
        if fichier_entrainement is not None:
            ...
            # Si aucune donnée réelle n'est chargée, générer des données fictives
            X_data = np.random.rand(200, 26)
            y_data = np.random.randint(0, 2, 200)
            st.session_state.models = train_models(X_data, y_data)
    ...
    # Prédiction via les modèles de classification
